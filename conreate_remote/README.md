...
On every startup with an enrolled device, the App log clearly prints `你的 Home Assistant 远程访问网址 / Remote access URL: ...` and the **HA 反向代理信任地址 / HA trusted proxy address**. The App also enables an administrator-only Home Assistant Ingress page: use **Open Web UI** to view, copy, or open the current URL and see pending/failed route changes. It is served only through HA Ingress on internal port `8099`, opens no new host/public port, and reads only `/config/status.json`.

The sanitized status file records the current phase, Device ID, remote URL, trusted proxy address, assignment revisions, route-change state, message, and update time; it never contains the device token, FRP credential, activation code, or private identity key. If HA returns HTTP 400, both the status page and file report `configuration_required` and point the administrator to Home Assistant's UI-based reverse-proxy settings with the exact address to copy, instead of asking the customer to edit YAML.
